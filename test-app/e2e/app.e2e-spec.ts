...
describe('Starting tests for test-app', function() {
  ...
  it('website title should be test-app', () => {
    page.navigateTo('/');
  ...
  it('network-name should be soychain@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('soychain@0.0.1.bna');
    });
  });

  it('navbar-brand should be test-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('test-app');
    });
  });
